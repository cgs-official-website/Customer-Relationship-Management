...
const LeadTable = ({ leads, onStatusChange, onDelete, onEdit, onView, usersMap = {}, selectedLeadIds = new Set(), setSelectedLeadIds }) => {
  ...
  const PER_PAGE = 10;
  const totalPages = Math.ceil(leads.length / PER_PAGE) || 1;
  const rows = leads.slice((page - 1) * PER_PAGE, page * PER_PAGE);
    ...
    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
        ...
        <table className="min-w-full border-collapse">
          <thead>
            <tr className="bg-gray-50/80 border-b border-gray-100">
              <th className="px-6 py-4 text-left whitespace-nowrap w-12">
                <input
                  type="checkbox"
              ...
              {['Lead Profile', 'Location', 'NEET Details', 'Campaign Source', 'Pipeline Status', 'Date', ''].map((h, i) => (
                <th key={i} className="px-6 py-4 text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap">
                  {h}
                </th>
              ))}
            </tr>
      ...
      <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-white">
        <p className="text-sm font-medium text-gray-500">
          Showing <span className="font-bold text-gray-900">{(page - 1) * PER_PAGE + (leads.length ? 1 : 0)}</span> to <span className="font-bold text-gray-900">{Math.min(page * PER_PAGE, leads.length)}</span> of <span className="font-bold text-gray-900">{leads.length}</span> leads
        </p>
        <div className="flex items-center gap-2">
